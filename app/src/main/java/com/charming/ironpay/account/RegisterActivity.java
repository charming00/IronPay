...
//        nameText = (EditText) findViewById(R.id.register_name);
        ...
        payPasswordText = (EditText) findViewById(R.id.register_paypassword);
        sendCheckNumBtn = (Button) findViewById(R.id.send_checknum);
...
//        params.add("username", nameText.getText().toString());
        try {
            params.add("paypassword",MD5.getMD5(payPasswordText.getText().toString()));
            params.add("password", MD5.getMD5(firstPasswordText.getText().toString()));
        } catch (NoSuchAlgorithmException e) {
            e.printStackTrace();
        }
        client.post(MyApplication.getServer() + "/register", params, new AsyncHttpResponseHandler() {
...
//        }
        ...
        if (payPasswordText.getText().toString().isEmpty()) {
            Toast.makeText(RegisterActivity.this, "支付密码不得为空", Toast.LENGTH_LONG).show();
            return;
        }
        ...
        if (firstPasswordText.getText().toString().length() > 16) {
            Toast.makeText(RegisterActivity.this, "密码位数不得大于16位", Toast.LENGTH_LONG).show();
            return;
        }
        if (firstPasswordText.getText().toString().length() < 6) {
            Toast.makeText(RegisterActivity.this, "密码位数不得小于6位", Toast.LENGTH_LONG).show();
            return;
        }

        if (payPasswordText.getText().toString().length() > 16) {
            Toast.makeText(RegisterActivity.this, "密码位数不得大于16位", Toast.LENGTH_LONG).show();
            return;
        }
        if (payPasswordText.getText().toString().length() < 6) {
            Toast.makeText(RegisterActivity.this, "密码位数不得小于6位", Toast.LENGTH_LONG).show();
            return;
        }
